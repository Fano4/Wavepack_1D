...
#GATHER DIPOLE MOMENT IN THE CATION STATES

function print_dipole_cation () {
...
do
  ...
  do
    ...
    ##############
        while [[ $temp1 -lt ${o} ]]
        do
           let ts1=${ts1}+${n_states_cat_sym[$temp1]} 
           let temp1=${temp1}+1
        done
    ##############
        let m=1 #initial state index in symetry o
        #loop over states indexes of the initial state in symmetry o
        while [[ $m -le ${n_states_cat_sym[$o]} ]]
        do
           let n=$m #state index of the final state in symmetry o
           let ts1=${ts1}+1 
           ...
           #loop over the states indexes of the final state in symmetry p
           while [[ $n -le ${n_states_cat_sym[$p]} ]]
           do
             let ts2=${ts2}+1
             echo "seeking for dipole moment of cation  ${n}.${pp}-${m}.${op} = ${ts2}-${ts1}"
             a=$(grep -m2 "<${n}.${pp}|DM${LL}|${m}.${op}>" ${fol}/${molpro_input}${R}.out | tail -n1| awk '{print $4}')

             #if there is no instance of the sought dipole, put it to zero
             if [[ -z $a ]]; then
               echo "0.0" >> ${output_loc}/LiH_cat_DM${LL}_${ts2}_${ts1}.input
             else
               echo "$a" >> ${output_loc}/LiH_cat_DM${LL}_${ts2}_${ts1}.input
             fi
...
#             fi
        ...
        while [[ $temp1 -lt ${p} ]]
        do
          let ts2=${ts2}+${n_states_cat_sym[$temp1]} 
          let temp1=${temp1}+1
        done
           ##############
        while [[ $n -le ${n_states_cat_sym[$p]} ]]
